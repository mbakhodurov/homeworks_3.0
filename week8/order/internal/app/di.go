...
import (
	...
	orderapiv1 "github.com/mbakhodurov/homeworks2/week8/order/internal/api/order/v1"
	iamclient "github.com/mbakhodurov/homeworks2/week8/order/internal/client/grpc/iam/v1"
	inventoryclient "github.com/mbakhodurov/homeworks2/week8/order/internal/client/grpc/inventory/v1"
	paymentclient "github.com/mbakhodurov/homeworks2/week8/order/internal/client/grpc/payment/v1"
	"github.com/mbakhodurov/homeworks2/week8/order/internal/config"
	assemblyconsumer "github.com/mbakhodurov/homeworks2/week8/order/internal/consumer/assembly_consumer"
	"github.com/mbakhodurov/homeworks2/week8/order/internal/interceptor"
	"github.com/mbakhodurov/homeworks2/week8/order/internal/middleware"
	orderpayproducer "github.com/mbakhodurov/homeworks2/week8/order/internal/producer/order_producer"
	orderrepo "github.com/mbakhodurov/homeworks2/week8/order/internal/repository/order"
	orderitemrepo "github.com/mbakhodurov/homeworks2/week8/order/internal/repository/order_item"
	ordersvc "github.com/mbakhodurov/homeworks2/week8/order/internal/service/order"
	"github.com/mbakhodurov/homeworks2/week8/platform/pkg/closer"
	...
	platformratelimit "github.com/mbakhodurov/homeworks2/week8/platform/pkg/ratelimit"
	...
	"github.com/mbakhodurov/homeworks2/week8/shared/static"
)
...
// RateLimitMiddleware возвращает HTTP middleware с распределённым rate limiter'ом.
// При первом вызове создаёт Redis-клиент и регистрирует его closer.
// Стратегия fail-open: если Redis недоступен — запрос пропускается.
func (d *diContainer) RateLimitMiddleware() func(http.Handler) http.Handler {
	if d.rateLimitMiddleware == nil {
		cfg := config.AppConfig().RateLimit
		rdb := redis.NewClient(&redis.Options{
			Addr: cfg.RedisAddress,
		})
		closer.Add("Redis rate limiter", func(_ context.Context) error {
			return rdb.Close()
		})
		limiter := redis_rate.NewLimiter(rdb)
		d.rateLimitMiddleware = platformratelimit.Middleware(limiter, cfg.Rate, cfg.Burst)
	}
	return d.rateLimitMiddleware
}
